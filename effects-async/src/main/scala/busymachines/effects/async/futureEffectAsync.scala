...
object FutureOps {
  ...
  /**
    * !!! USE WITH CARE !!!
    * Most likely you want to use [[suspendInIO]] which ensures that it suspends
  ...
  /**
    * !!! USE WITH CARE !!!
    * Most likely you want to use [[suspendInTask]] which ensures that it suspends
    * the side effects of the given future (if it created by the given expression,
    * if it's a val, then good luck).
    *
    * Having to care about such val/def distinctions shows why Future is an
  ...
  /**
    * Suspends the side-effects captured within the Future, and executes them
    * in an [[IO]]
    *
    * Be careful that this works properly only if you have full control over
    * the creation of Future, and you are sure that it is the result of a "def"
    * call, that has not yet started the side-effecting operation of [[Future.apply]]
    */
  def suspendInIO[T](value: => Future[T])(implicit ec: ExecutionContext): IO[T] =
