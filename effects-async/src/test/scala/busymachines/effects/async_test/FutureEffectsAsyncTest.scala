...
/**
  ...
  *
  */
final class FutureEffectsAsyncTest extends AnyFunSpec {
  implicit val ec: ExecutionContext = ExecutionContext.global
  //prevents atrocious English
  private def test: ItWord = it

  implicit private class TestSyntax[T](value: Future[T]) {
    //short for "run"
    ...
    describe("Future — companion object syntax") {
      ...
      describe("constructors") {
        ...
        describe("fromEitherThr") {
          test("left — transform") {
            assertThrows[IllegalArgumentException](Future.fromEitherThr(left, (_: Throwable) => iae).r)
          }

          test("right") {
            assert(Future.fromEitherThr(right, (_: Throwable) => iae).r == 42)
          }
        }
