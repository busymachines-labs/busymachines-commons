catalogManager.config(["$locationProvider", "$httpProvider", function($locationProvider, $httpProvider) {
    $locationProvider.html5Mode(true);
    $httpProvider.responseInterceptors.push('AuthResponseInterceptor');
    $httpProvider.responseInterceptors.push('SpinnerInterceptor');
}]);

catalogManager.run(["$rootScope", "$location", "$http", "Cookies", "parties", "RequestCounter", "$locale", function($rootScope, $location, $http, Cookies, parties, RequestCounter, $locale) {
