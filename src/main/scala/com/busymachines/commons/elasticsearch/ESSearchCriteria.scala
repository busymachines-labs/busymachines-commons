...
object ESSearchCriteria {
  ...
  case class Not[A](child: ESSearchCriteria[A]) extends ESSearchCriteria[A] {
    def toFilter = FilterBuilders.notFilter(child.toFilter)
    def prepend[A0](path : ESPath[A0, A]) = Not(child.prepend(path))
  }
  ...
  case class Neq[A, T](path: ESPath[A, T], value: T) extends ESSearchCriteria[A] {
    def toFilter = Not(Equ[A, T](path, value)).toFilter
